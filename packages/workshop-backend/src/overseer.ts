    ...
    try {
      ...
      while (true) {
        ...
        try {
          ...
          // The turn_end barrier consumes every action associated with a persisted model step. Any
          // remainder came from work before a barrier and must not attach to a later run; automatic
          // compaction re-enters this loop immediately.
          this.consumeCapturedActions(chatId);
        }
